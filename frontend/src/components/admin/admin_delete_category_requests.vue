<template>
    ...
    <div v-else>
        ...
        <div class="card" style="margin-top: 20px;">
            <div class="card-body">
                <div class="row">
                    <h3 class="card-title col">Delete Category Requests</h3>
...
<script>
    ...
    export default {
        name: "Admin_Delete_Category_Requests",
        components: {
            Un_Authorized,
            Admin_Nav
        },
        ...
        methods: {
            async accept(e){
                let message=`Are you sure you want to Accept this Request? Please note that accepting this request will Delete the Category and all Requests of that Category.`
                if (!confirm(message))
                {return}
                ...
                {
                    if (request.category_id==this.requests[i].category_id)
                    {
                        this.requests.splice(i, 1)
                    }
                    else
                ...
                this.msg="Request Accepted! Category Deleted Successfully!"
            ...
            async decline(e){
                ...
                let access_token=this.$store.state.access_token
                let r_id=e.target.classList[3]
                let url=`http://127.0.0.1:5000/api/admin/${a_id}/delete_category_request/${r_id}`
        ...
        async mounted(){
            ...
            let url=`http://127.0.0.1:5000/api/admin/${a_id}/delete_category_requests`
            let resp=await fetch(url, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
